...
class Transcriber:
    def __init__(self, model_size: str = "base", language: str = "en"):
        ...
        if model_size == "tiny":
            ...
            self.final_model = WhisperModel("tiny", device="cpu", compute_type="auto")
            self.stream_model = self.final_model
            print("done.")
        else:
            print("Loading streaming model (tiny)... ", end="", flush=True)
            self.stream_model = WhisperModel("tiny", device="cpu", compute_type="auto")
            print("done.")
            print(f"Loading final model ({model_size})... ", end="", flush=True)
            self.final_model = WhisperModel(model_size, device="cpu", compute_type="auto")
            print("done.")
